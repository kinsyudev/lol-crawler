...
impl RiotApiClient {
    pub fn new(config: Config, rate_limiter: Arc<RateLimiter>, database: Database) -> Result<Self, ApiError> {
    ...
    async fn make_request(&self, url: &str, region: &str) -> Result<Response, ApiError> {
        let endpoint = url.split(&self.config.base_url_for_region(region))
            .nth(1)
            .or_else(|| url.split(&self.config.regional_base_url_for_region(region)).nth(1))
            .unwrap_or(url);
        ...
        self.rate_limiter.acquire_permit(endpoint, region).await
        ...
        self.rate_limiter.update_limits_from_headers(endpoint, region, response.headers()).await;

        match response.status().as_u16() {
            200 => Ok(response),
            400 => Err(ApiError::BadRequest(response.text().await.unwrap_or_default())),
    ...
    pub async fn get_featured_games(&self, region: &str) -> Result<FeaturedGamesResponse, ApiError> {
    ...
    pub async fn get_summoner_by_name(&self, region: &str, summoner_name: &str) -> Result<SummonerResponse, ApiError> {
        let url = Endpoints::summoner_by_name(&self.config, region, summoner_name);
        log::debug!("Fetching summoner by name: {} in region: {}", summoner_name, region);
        self.make_request_with_retry(&url, region).await
    }

    pub async fn get_summoner_by_puuid(&self, region: &str, puuid: &str) -> Result<SummonerResponse, ApiError> {
        let url = Endpoints::summoner_by_puuid(&self.config, region, puuid);
        log::debug!("Fetching summoner by PUUID: {} in region: {}", puuid, region);
        self.make_request_with_retry(&url, region).await
    }

    pub async fn get_summoner_by_id(&self, region: &str, summoner_id: &str) -> Result<SummonerResponse, ApiError> {
        let url = Endpoints::summoner_by_id(&self.config, region, summoner_id);
        log::debug!("Fetching summoner by ID: {} in region: {}", summoner_id, region);
        self.make_request_with_retry(&url, region).await
    }
    ...
    pub async fn get_match_list_by_puuid(
        ...
        log::debug!("Fetching match list for PUUID: {} in region: {}", puuid, region);
        self.make_request_with_retry(&url, region).await
    }

    pub async fn get_match_by_id(&self, region: &str, match_id: &str) -> Result<MatchDto, ApiError> {
        let url = Endpoints::match_by_id(&self.config, region, match_id);
        log::debug!("Fetching match: {} in region: {}", match_id, region);
        self.make_request_with_retry(&url, region).await
    }

    pub async fn get_master_league(&self, region: &str, queue: &str) -> Result<LeagueListResponse, ApiError> {
        let url = Endpoints::master_league(&self.config, region, queue);
        log::debug!("Fetching master league for queue: {} in region: {}", queue, region);
        self.make_request_with_retry(&url, region).await
    }

    pub async fn get_grandmaster_league(&self, region: &str, queue: &str) -> Result<LeagueListResponse, ApiError> {
        let url = Endpoints::grandmaster_league(&self.config, region, queue);
        log::debug!("Fetching grandmaster league for queue: {} in region: {}", queue, region);
        self.make_request_with_retry(&url, region).await
    }

    pub async fn get_challenger_league(&self, region: &str, queue: &str) -> Result<LeagueListResponse, ApiError> {
        let url = Endpoints::challenger_league(&self.config, region, queue);
        log::debug!("Fetching challenger league for queue: {} in region: {}", queue, region);
        self.make_request_with_retry(&url, region).await
    }
